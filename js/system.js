	...
	$.getJSON(ajaxPath + "?act=upd&id=" + id + "&q=" + q + "&site_id=" + SITE_ID, function(data) {
		if(data["success"] == "Y"){
            ...
            if(parseInt(data["DISCOUNT_PRICE"], 10) > 0)
                priceTextClass = 'hasDiscount';
            
			var $price = $appBasket.find(".price").html('<span class="'+priceTextClass+'">'+data["PRICE"]+'</span>').data({"price": data["~PRICE"], "discount": data["OLD_PRICE"]});
...
$(window).on("ready", function(event){
	...
	var displayWindow = function(jData){
		if($(window).width() > 700){
            ...
            var priceTextClass = '';
            if(parseInt(jData["DISCOUNT_SUM"], 10) > 0 && typeof(jData["MARKER_OFFERS"]) != 'undefined' && jData["MARKER_OFFERS"].indexOf("Р Р°СЃРїСЂРѕРґР°Р¶Р°") > -1)
                priceTextClass = 'hasDiscount';
			var $price = $appBasket.find(".price").html('<span class="'+priceTextClass+'">' + jData["PRICE"] + '</span>').data({"price": jData["~PRICE"], "discount": jData["OLD_PRICE"]});
			...
			var $count_str = $appBasket.find(".count_str").data("id", jData["ID"]);
			...
			$count_str.html(jData["COUNT_STR"]);
  ...
  if (!$(this).data('status')) {
    ...
    $(this).data('status', true);
  	...
  	$(this).html('РџРѕСЃРјРѕС‚СЂРµС‚СЊ РїРѕРґСЂРѕР±РЅРµРµ');
...
/* $('.btn_add').on("click", function (){
 	$(document).find('.block_popup_full').addClass('active');
 });

 $('.close_popup').on("click", function (){
 	$(document).find('.block_popup_full.active').removeClass('active');
 });
 */


 $('.btn_add[name=add-item-form-button]').on("click", function (){
 	$(this).parent().toggleClass('none');
 });


$('.btn_add').on('click', function() {
	openPopup($(".block_popup_full, .body_popup, .md-overlay"));
});
...
function openPopup(obj){
	...
	var h = obj.find(".body_popup").height();
	obj.hide().css({"visibility":"visible"});
	var top =  '150px' + $(window).scrollTop();
	obj.find(".body_popup").css({"top":top});
	obj.fadeIn();
}
	
function closePopup(){
	$(".block_popup_full").hide();
	$(".md-overlay").hide();
}
